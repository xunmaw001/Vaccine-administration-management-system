    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="身体状态" prop="shentizhuangtai">
          <el-select :disabled="ro.shentizhuangtai" v-model="ruleForm.shentizhuangtai" placeholder="请选择身体状态">
            <el-option
...
export default {
  ...
  created() {
	...
	this.ruleForm.likaishijian = this.getCurDateTime()
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `liuguandengji/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.liuguandengjiCrossAddOrUpdateFlag = false;
						 this.parent.search();
